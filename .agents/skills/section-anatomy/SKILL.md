...
- **The page builder column owns the rhythm between sections and nothing else.** `PageSections.astro` is `flex w-full flex-col gap-(--section-gap) py-(--section-block)`. A section adds its own internal padding when its design calls for it, but it does not restate the gap.
- **A section spends the rhythm ladder rather than writing numbers.** `--section-block` between two blocks of the section, `--section-group` between repeated peers, and `SectionHeader.astro` for the eyebrow/rule/headline opening (it owns `--section-rule` and `--section-header`). Nothing inside a section may reach `--section-gap`: the moment it does, the boundary between two sections stops reading as the widest break on the page, which is what the ladder is for. See `tailwind`.
- **A section that paints its own background takes `-my-(--section-surface-inset)`** and spends `py-(--section-block)` inside. Its colour edge is already the boundary, so the full page gap plus its own padding reads as a hole (`IntersectionSection`, `TeamSection`'s stage).
...
- `src/features/page-builder/SectionHeader.astro`: the shared eyebrow/rule/headline opening and the two distances inside it; `tone` picks accent, muted, or inverse (on a dark surface).
